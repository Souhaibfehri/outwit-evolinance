...
export function NotificationsDropdown() {
  const [notifications, setNotifications] = useState<SimpleNotification[]>(sampleNotifications)
  const [isOpen, setIsOpen] = useState(false)

  const unreadCount = notifications.filter(n => !n.read).length

  const getNotificationIcon = (title: string) => {
    if (title.includes('Bill')) return <Calendar className="h-4 w-4 text-yellow-500" />
    if (title.includes('Budget')) return <AlertTriangle className="h-4 w-4 text-orange-500" />
    if (title.includes('Goal')) return <Target className="h-4 w-4 text-blue-500" />
